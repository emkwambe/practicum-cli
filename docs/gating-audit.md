...
- **`role` field (ADDED `838c5b1`):** present on every license record and returned by `/license/validate` (`"learner"` for all individual and team purchases, `"instructor-admin"` for Classroom only; older records read back as `"learner"`). The CLI does not store or consume it yet — `_license_write` in `lib/license.sh` caches only the fields listed above. No instructor dashboard exists — audit item 7 still open.
