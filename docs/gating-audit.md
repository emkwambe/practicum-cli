...
- **`role` field (ADDED `838c5b1`):** present on every license record and returned by `/license/validate` (`"learner"` for all individual and team purchases, `"instructor-admin"` for Classroom only; older records read back as `"learner"`). The CLI caches it in `~/.practicum/license.json` (`role`, readable via `_license_field role`) but does not consume it yet. No instructor dashboard exists — audit item 7 still open.
