...
**Date:** 2026-09-21
**Version:** v3.1.0 (`VERSION` file, read into `PRACTICUM_VERSION` — item 11)
...
5. ~~**Certificates are ungated.**~~ **RESOLVED in `b481672`.** `cmd_certificate` now calls `can_access_course "$(get_active_course)"` first; the same commit gates `cmd_final` (day5), `cmd_assess2` (day9) and `cmd_final_capstone` (day10), which were also reachable on the free tier. CLI Immersion still issues certificates (free course passes `can_access_course`). *(Was: `cmd_certificate` (`practicum:748`) only requires the free Day 1 quiz. Combined with (4), a free learner can produce a full-course certificate.)*
...
9. ~~**Scores are not namespaced per course.**~~ **RESOLVED in `576763e`.** `scores.txt` keys are now `<course-slug>:<key>=<value>`, applied centrally in `_score_key` (`lib/state.sh`) since every call site already went through `save_score`/`get_score`. `migrate_scores` prefixes legacy files with the active slug at startup — idempotent, leaves namespaced lines alone. Verified: a Day 4 pass in Linux Foundations reads as empty in Kubernetes. *(Was: `scores.txt` keys collide across courses, so passing the Day 4 quiz in Linux Foundations marks it passed in Kubernetes. Affects the Labs gate and certificate accuracy.)*
   **Status: OPEN** (re-checked 2026-09-22). `~/.practicum/scores.txt` keys remain unprefixed, so `day4_quiz` passed in Linux Foundations reads as passed in Kubernetes. **Fix:** key scores as `<course-slug>:<key>`, plus a one-time migration for existing `scores.txt` files. *(A sprint once targeted this item as "unlock chain coverage"; that gate audit was done in `b481672` and is recorded as item 13 — a different problem.)*
...
11. ~~**Version drift.**~~ **RESOLVED in `b481672`.** A single `VERSION` file at the repo root (`3.1.0`, matching the highest git tag) is read into `PRACTICUM_VERSION` and printed by `--help`; the hardcoded `3.0.0` strings are gone. *(Was: `practicum` reports 3.0.0; git history is at v3.1.0.)*
12. ~~**Local cache is unsigned (residual of item 2).**~~ **RESOLVED in `576763e`.** The cache carries `cache_sig`: HMAC-SHA256 over the rest of the file, keyed on the license key itself (no new secrets). `validate_license` verifies it before trusting any cached field; on mismatch it re-fetches with the stored key and rewrites the cache, or purges and blocks when the server is unreachable. Verified: hand-widened `entitlements` are reverted by revalidation; a from-scratch forged file is rejected. **Residual:** without `openssl` the check degrades to the epoch window only (one-time note on stderr) — access is never blocked for the missing tool, so a forged cache still works for the 24 h TTL on such systems. *(Was: `~/.practicum/license.json` can be hand-written with a fresh `cached_epoch` and re-dated daily. Low severity now that item 1 (content in the repo) is the easier bypass, but should be closed together with it.)*
13. ~~**Assessment and certificate commands were ungated.**~~ **RESOLVED in `b481672`.** Four CLI subcommands delivered premium content without any license check: `cmd_final` (Day 5 assessment), `cmd_assess2` (Day 9 Assessment 2), `cmd_final_capstone` (Day 10 capstone) and `cmd_certificate` (full-course certificate). The three assessments now call `can_access_day` for their day; `cmd_certificate` calls `can_access_course "$(get_active_course)"`. CLI Immersion is unaffected (free course passes the check). Found while auditing lesson-command gate coverage — the menu-driven `cmd_dayN_menu` handlers were already gated at `cmd_start`, and `wizard`/`lab`/`field` only switch mode before re-entering it.
...
**Changelog:**
- 2026-09-22 — items 9 (score namespacing + migration) and 12 (signed license cache) resolved in `576763e`; stale 3.0.0 header note corrected.
- 2026-09-22 — items 5, 11 and new item 13 resolved in `b481672` (certificate + Day 5/9/10 assessment gates; single VERSION file). Item 9 (score namespacing) re-checked and still open.
