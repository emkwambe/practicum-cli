...
9. **Scores are not namespaced per course.** `scores.txt` keys collide across courses, so passing the Day 4 quiz in Linux Foundations marks it passed in Kubernetes. Affects the Labs gate and certificate accuracy.
   **Note (2026-09-22):** a sprint targeted this item as "unlock chain coverage" and the gate audit it asked for was done in `b481672` (see item 5) — but that is a different problem. The score-collision bug itself is **still OPEN**: `~/.practicum/scores.txt` keys remain unprefixed, so `day4_quiz` passed in one course reads as passed in every course. Fix is to key scores as `<course-slug>:<key>` with a one-time migration of existing files.
...
**Changelog:**
- 2026-09-22 — items 5 and 11 resolved in `b481672` (certificate + day5/9/10 assessment gates; single VERSION file). Item 9 re-checked and still open.
