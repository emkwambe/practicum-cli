...
**The dashboard is served by the existing Pages site.** Build it with whatever the Pages site already uses; if it is static HTML, a single module-based page set under `/dashboard` with `fetch(..., { credentials: 'include' })` is sufficient. CORS on the Worker allows `https://practicum-cli.dev` with credentials. It must be responsive and usable on a tablet, since instructors often check progress from the front of a room.
